...
\begin{enumerate}
	...
	\item  The $ y _{ bsv } $ predict what the next token will be, i.e. $ x _{ bs+1 } $, having seen the \textbf{context}
          ...
          simplicity, a \pyinline{Softmax}  of $ y _{ bsv } $ gives the conditional probability $ p _{ bsv
...
\textit{totally redundant} if \pyinline{LayerNorm} is immediately followed by a \pyinline{Linear}
...
Note that the losses for all possible context lengths are included in the sum\footnote{In Natural Language Processing (NLP),
	the \pyinline{perplexity} is often reported instead of the loss, which is just the exponential of the loss, a geometric-mean over the
...
left holding a \pyinline{(B, S, D)}-shaped tensor which must be \pyinline{AllReduce}-d over the
\pyinline{D}-dimension. Note that no \pyinline{AllReduce} is needed for the
...
the final output matrix multiply can be schematically broken up as in \begin{align}
& {\rm Concat} \left ( \left [ t ^{ 0 }, \ldots , t ^{ H-1 } \right ] \right ) \cdot
O \nn & = {\rm Concat} \left ( \left [ {\rm Concat}\left ( \left [ t ^{ 0 },
        \ldots , t ^{ \frac{ A }{ T }-1 } \right ] \right ) \cdot O ^{
        (0) }, \ldots ,{\rm Concat}\left ( \left [ t ^{ H-\frac{ A }{ T
                } }, \ldots , t ^{ A} \right ] \right ) \cdot O ^{ (T-1)
} \right ] \right ) \ ,\end{align} where matrix products and
concatenation both occur along the hidden dimension. That is, each
worker in a group has $ A/T $ different \pyinline{(B, S, D/A)}-shaped
activations $ t ^{ a } $, which can be concatenated into a \pyinline{(B,
S, D/T)}-shaped tensor and multiplied into the \pyinline{(D/T,
D)}-shaped shard of $ O $, such that every worker is again left with a
\pyinline{(B, S, D)}-shaped shard which much be \pyinline{AllReduce}-d,
exactly as in the \pyinline{MLP} case above, to get the final result.
Hence, the collective communication costs for both layers are the same.
The backwards pass requires similar collective communications to the
\pyinline{MLP} case above.
...
\begin{itemize}
	...
	\item \pyinline{AllGather}: all data $ x ^{ (n) } $ is communicated to all workers. Functionally
	      equivalent to a \pyinline{Gather} followed by \pyinline{Broadcast}.
	...
	\item \pyinline{ReduceScatter}: a reducing operation (\pyinline{sum}, \pyinline{mean}, etc.) is
...
\begin{itemize}
	\item \textbf{Threads}  are the fundamental unit of execution which each run the same CUDA
	      ...
	      \cudainline{threadIdx.{x, y, z}} attributes with \cudainline{threadIdx.x} in \cudainline{0, ..., blockDim.x -1}
	      and similar.
