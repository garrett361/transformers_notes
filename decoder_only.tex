...
\State Move  $ q _{ irh }$ on-chip, instantiate temps $Z _{ irjh }, L _{ irj }$ to zeros and $M _{ irj }$ to $ -\infty $ on-chip.
\For{ $ j \in \ldots  $}
    ...
    \State $ M _{ irj }  \gets \max(M _{ ir(j-1) },  \max _{ c } S _{ irjc } ) $ \Comment Get max attention score seen so far
    ...
    \State $ Z _{ irjh } \gets   e^{ M _{ ir } - M _{ irj } }Z _{ irjh } +\tilde{P} _{ irjc}v _{ jch }$ \Comment Correct, update numerators
\EndFor
\State $ z _{ irh } \gets \frac{1}{L _{ ir(-1) }} Z _{ ir(-1)h }$ \Comment Divide $ z $ by last $ j  $ value of $ L _{ irj } $
    \State $ \ell _{ ir } \gets M _{ ir(-1) }+\ln L _{ ir(-1) }$ \Comment Store statistics for bwd
...
\For{ $ w \in \left \{ r, \ldots , R-1, 0, \ldots , r-1 \right \}  $} \Comment Computing components $ z _{ \bar{r}td } $ $ \forall t, d $
    ...
    \EndIf
    \State $ Z _{ \bar{r}td } \gets Z _{ \bar{r}td }+   \exp \left ( q _{ \bar{r}t d' } k _{ \bar{w}x d' } \right ) $
    \State $ L _{ \bar{r}t } \gets  L _{ \bar{r}t } +  \sum _{ x } q _{ \bar{r}t d } k _{ \bar{w}x d }  $
\EndFor
\State $ z _{ \bar{r}td } \gets \frac{ Z _{ \bar{r}td } }{ L _{ \bar{r}t }  }$
\label{ring_attn_fwd_naive}
\end{algo}
