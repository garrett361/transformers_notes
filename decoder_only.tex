...
\pyinline{Softmax} is another instance where this occurs, since
\begin{align}
...
_{ d e }= W ^{ 0 }_{ d (f t) } \equiv  \bar{W} ^{ 0 } _{ d f \bar{t} }$ (again in \pyinline{einops}-like
...
their backward pass, and in the backwards pass \pyinline{AllReduce} operation (see
...
computed by an \pyinline{AllReduce} } to collect gradient shards from other.
...
when convenient below). The \pyinline{(D, D/A, A)} query, key, and value weight matrices $ Q _{ dea
...
\pyinline{(B, S, D/A, A/T)}-shaped slice of the \pyinline{(B, S,D/A,A/T,T)}-shaped re-weighted
...
\pyinline{AllReduce}-d\footnote{The amount of communicated data is again $ \Ocal \left( BSD
\right)$. } to give every worker the full result above, $z _{ sd'}$. We neglected
\pyinline{Dropout} here for simplicity. The collective communications story is essentially
equivalent to that of the \pyinline{MLP} layers: one \pyinline{AllReduce} is needed in the forwards pass
...
In more detail:
\begin{itemize}
    \item The \pyinline{LayerNorm}s can act on $ z _{ sd } $ sharded across the sequence dimension
        as $ z _{ r \bar{t}d } $
    \item The self-attention layers require coalescing the previous
        output back $ z _{ r \bar{t}d }\longrightarrow z _{ sd } $ via an
        \pyinline{AllGather}\footnote{A small memory optimization is to have each rank free all but
        one shard of this activation after using it and then re-materialize it with an
    \pyinline{AllGather} in the backwards-pass.}. This is also convenient to do before the MLP layers,
    but other options are possible.
    \item Rather than \pyinline{AllReduce} the self-attention and MLP outputs over the sharded
    ...
    as in $ y _{ sd \bar{t} } = z _{ se \bar{t} } W _{ e \bar{t}d }$ and \pyinline{ReduceScatter}
...
what the next  \pyinline{LayerNorm} expects.
\end{itemize}
...
shape \pyinline{M.shape=(A, 2)}: sharding this tensor across two devices across the final index
results in a tensor $ \bar{M}_{ a \bar{b} } $ which is of shape \pyinline{M_bar.shape=(A, 1)} on
