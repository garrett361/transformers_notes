...
layer, as it is in the standard transformers architecture. The scaling the biases performed by these
...
\begin{align}
    t _{FLOPs} &=  \frac{ F _{ {\rm total}  } ^{ {\rm  model}  }  }{ \lambda _{ {\rm FLOP/s} } } \approx \frac{ 2 BSN _{ {\rm params} } }{ \lambda _{ {\rm FLOP/s} } }\ .
\end{align}
...
now memory-bound (again, see App.~\ref{app_compute_mem_bound}) and has per-token-latency of
...
In more detail, the \pyinline{MLP} layers just process $ S=1 $ length tensors, but are insensitive
...
\begin{align}
    N _{ {\rm params} } &=  \frac{ (4+2E)LD ^{ 2 } }{  T} + VD+ \Ocal \left( DL \right) \approx  \frac{ (4+2E)LD ^{ 2 } }{  T} \ ,
\end{align}
