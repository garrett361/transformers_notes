...
the omitted constant may be relatively large, but still negligible when all dimensions are big.}. It is
...
\section{The Minimum Needed and the kv-Cache }
...
\begin{itemize}
    ...
    \item Every component of the key and value vectors $k ^{ a }_{ bsd }, v ^{ a }_{ bsd } $ is
        ...
        we are on, then $ z ^{ (t+1) } _{ bsd } = z ^{ (t)} _{ b (s-1)d } $ for every tensor in the network, as long as $ s $ isn't the final index.}
\end{itemize}
...
\begin{align}
    M _{ {\rm kv-cache}  } &= 2pBSD \ . \label{eq_kv_cache_memory}
\end{align}
This can easily be larger than the memory costs of the model parameter: $ M _{ {\rm params}  } ^{
{\rm  inference}  } \sim p N _{ {\rm params}  } \sim p LD ^{ 2 }  $ (dropping $ \Ocal \left( 1
\right)  $ factors), so that the cache takes up more memory when $ BS \gtrsim LD $.





\section{Memory and FLOPs}
...
\subsection{NVIDIA GPU Stats}


The most important statistics for GPUs are their memory capacity, their maximum (theoretical) FLOP/s
rate, their memory bandwidth, and their \textit{math bandwidth}
(\href{https://docs.nvidia.com/deeplearning/performance/dl-performance-gpu-background/index.html#gpu-arch}{which
is the ratio of the last two quantities}):

\begin{center}
\begin{tabular}{| c |c| c |c |c| }\hline
    GPU & Memory & FLOP/s  & Memory Bandwidth (B/s)  & Math Bandwidth (FLOPs/B)\\ \hline
    A100 & 40GiB/80GiB & $ 312 \times 10 ^{ 12 } $ & $  1.5 \times 10 ^{  12 }   $ & 208 \\ \hline
\end{tabular}
\end{center}
