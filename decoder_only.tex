...
\pyfile[firstline=8,lastline=68]{python/causal_attention.py}
...
\pyinline{(D/T, D)}-shaped shard of $ O $, such that every worker is again left with a
\pyinline{(B, S, D)}-shaped shard which much be \pyinline{AllReduce}-d, exactly as in the
\pyinline{MLP} case above, to get the final result.  Hence, the collective communication costs for both layers are the same.
The backwards pass requires similar collective communications to the \pyinline{MLP}
case above.
