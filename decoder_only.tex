...
\pyinline{(d0, ... , dn, I)}-shaped with $N= \prod _{ i } d_{i  } $ total elements let $ W $ be
\pyinline{(D, E)}-shaped such that it acts on the last index of $ z $, making $ z' $
\pyinline{(d0, ... , dn, J)}-shaped. The forward-FLOPs cost of this operation is therefore $ 2NJ $.
...
Adding indices, the \pyinline{(D, E)}-shaped weight derivative is
\begin{align}
