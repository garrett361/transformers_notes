...
The \pyinline{LayerNorm} operations acts over the hidden dimension. Spelling it out, given the
input tensor $ z _{ bsd } $ whose mean and variance over the $ d $-index are $ \mu _{ bs } $ and $
	\sigma _{ bs } $, respectively, the \pyinline{LayerNorm} output is
\begin{align}
